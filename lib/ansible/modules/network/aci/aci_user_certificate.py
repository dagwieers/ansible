...
DOCUMENTATION = r'''
---
module: aci_user_certificate
short_description: Manage user certificates (aaa:UserCert)
description:
...
options:
  ...
  certificate:
    description:
    - The X.509 certificate content.
    aliases: [ cert_data, certificate_data ]
  certificate_name:
    description:
    - The name of the certificate entry in ACI.
    aliases: [ cert_name ]
  user_type:
    description:
...
def main():
    ...
    module = AnsibleModule(
        ...
        required_if=[
            ['state', 'absent', ['user', 'certificate', 'certificate_name']],
    ...
    aci.construct_url(
        root_class=dict(
            ...
            filter_target='eq({}.name, "{}")'.format(ACI_MAPPING[user_type]['aci_class'], user),
            module_object=user,
        ),
        subclass_1=dict(
            aci_class='aaaUserCert',
            aci_rn='usercert-{}'.format(certificate_name),
            filter_target='eq(aaaUserCert.name, "{}")'.format(certificate_name),
