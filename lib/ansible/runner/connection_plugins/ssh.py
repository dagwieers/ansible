...
class Connection(object):
    ...
    def exec_command(self, cmd, tmp_path, sudo_user, sudoable=False, executable='/bin/sh', pty=False):
        ...
        if not self.runner.sudo or not sudoable:
            if pty:
                ssh_cmd += ['ssh', '-q', '-x', '-tt'] + self.common_args + [self.host, executable + ' -c ' + pipes.quote(cmd)]
            else:
                ssh_cmd += ['ssh', '-q', '-x', '-T'] + self.common_args + [self.host, executable + ' -c ' + pipes.quote(cmd)]
        else:
            ...
            ssh_cmd += ['ssh', '-q', '-x', '-tt'] + self.common_args + [self.host, '/bin/sh -c ' + pipes.quote(sudocmd)]
